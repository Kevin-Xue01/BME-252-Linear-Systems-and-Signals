...
subplot(211);
stem(xa,a,'LineWidth',3); hold on; stem(xb,b,'Color',red,'LineWidth',3);
xlim([-length(b) mlen]);
M(1) = getframe;
...
for m = 1:mlen-1
    ...
    subplot(212); hold off; stem(conv,'k','LineWidth',3); xlim([-length(b) mlen]);
    xb = xb+1;
    pause(pausespacing);
    subplot(211); hold off; stem(xa,a,'LineWidth',3); hold on;stem(xb,b,'Color',red,'LineWidth',3);xlim([-length(b) mlen]);
    M(m+2) = getframe;
end